...
app.showKeyvisual = function (keyvisual) {
	...
	keyvisual.data.forEach(function (item, index) {
		// create circle
		let circle = app.createElement("a", {
			atrs: {
		...
		let visual = app.createElement("a", {
			atrs: {
				...
				// 這邊看要不要請後端直接把JSON的連結網址改掉
				// https://api.appworks-school.tw/assets/keyvisuals/201807242228.jpg
...
app.showProducts = function (data) {
	...
	if (data.length === 0) {
		...
		for (let i = 0; i < data.length; i++) {
			let product = data[i];
			let productContainer = app.createElement("a", {
				atrs: {
...
app.showProduct = function (product, container) {
	...
	// colors
	let colorsContainer = app.createElement("div", {
		atrs: {
			className: "colors"
		}
	}, container);
